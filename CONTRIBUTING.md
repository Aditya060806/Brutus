...
BRUTUS is an ambitious, kernel-level OS agent, and building the future of human-computer interaction is a massive undertaking. Right now, this is a solo-developed project, which means your contributions are incredibly valuable—but my time to review them is limited.

Please read this guide to ensure your Pull Requests (PRs) merge smoothly and keep the codebase pristine.

## 🔗 Quick Links

- **GitHub:** https://github.com/Aditya060806/Brutus
- **LinkedIn:** [Aditya Pandey](https://www.linkedin.com/in/aditya-pandey-p1002/)

## 👑 Maintainers

- **Aditya Pandey** - Creator & Lead Architect
...
2. **New features / architecture** → Start a [GitHub Discussion](https://github.com/Aditya060806/Brutus/discussions) or open an Issue first. Please don't spend 20 hours building a massive feature without checking if it aligns with the project roadmap.
...
- Ensure both the **Main Process (Node.js)** and **Renderer Process (React)** compile without errors:
  - `npm run build`
- **Mind the Bridge:** BRUTUS operates on a strict split-architecture. Frontend React code cannot use Node.js modules (like `fs` or `child_process`). All system-level execution MUST be handled in the backend and triggered via the `window.electron.ipcRenderer.invoke` bridge.
...
Since BRUTUS is an AI-first operating system, AI-assisted code is treated as a first-class citizen. I just want transparency so I know how to review it.
...
BRUTUS is growing, and I am selectively looking to expand the maintainer team. If you are an elite developer who understands Electron, React, or local LLM execution, I'd love to have you on board.

Being a maintainer is about consistent involvement—triaging issues, reviewing PRs, and driving the architecture forward.

If you've successfully merged a few PRs and want to step up, reach out via LinkedIn or open an issue titled `[Maintainer Application]`. We are looking for people skilled in:
...
Because BRUTUS handles direct OS-level execution and local vault decryption, security is the highest priority.
