    ...
    async (event, { query, tavilyKey, notionKey, notionDbId, groqKey }) => {
      try {
        if (!tavilyKey || !groqKey) {
          throw new Error(
            'Missing API Keys. Please configure your Tavily and Groq keys in Settings → API Keys.'
          )
        }

        event.sender.send('oracle-progress', {
          status: 'scanning',
          file: 'BRUTUS and Tavily Neural Search Active...',
          totalFound: 1
        })
      ...
      `
        ...
        // Notion save is optional — only runs if both keys are provided
        if (notionKey && notionDbId) {
          try {
            ...
            await notion.pages.create({
              ...
              properties: { Name: { title: [{ text: { content: `BRUTUS Deep-Dive: ${query}` } }] } },
              children: notionChildren
            })
          } catch (notionErr) {
            // Notion failed but research data is still valid — don't terminate
            console.warn('[DeepResearch] Notion write failed (non-fatal):', notionErr)
          }
        }
