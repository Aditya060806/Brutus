...
/** One entry in `userData/Connected Devices/Connect-mobile.json`. */
...
export default function registerAdbHandlers(ipcMain: IpcMain) {
  ...
  const readHistory = async (): Promise<SavedDevice[]> => {
    try {
      const file = await fs.readFile(historyPath, 'utf-8')
      const parsed = JSON.parse(file)
      return Array.isArray(parsed) ? parsed : []
    } catch (_e) {
      // No file yet, or it was corrupted by a partial write. Either way an
      // empty list is the correct answer — the caller renders "no devices".
      return []
    }
  }

  ipcMain.removeHandler('adb-get-history')
  ipcMain.handle('adb-get-history', readHistory)

  /**
   * Add a device without connecting to it.
   *
   * Saving used to be a side effect of a *successful* connect, so a phone you
   * could not reach right now — asleep, on another network, ADB daemon not
   * started — could never be added to the list at all. You had to get it online
   * first to be allowed to save it, which is backwards.
   */
  ipcMain.removeHandler('adb-save-device')
  ipcMain.handle('adb-save-device', async (_, { ip, port, model }) => {
    const cleanIp = String(ip ?? '').trim()
    const cleanPort = String(port ?? '').trim() || '5555'
    if (!cleanIp) return { success: false, error: 'An IP address is required.' }

    await saveDeviceToHistory(cleanIp, cleanPort, String(model ?? '').trim() || 'Saved device')
    return { success: true, devices: await readHistory() }
  })

  /**
   * Remove a device from the list.
   *
   * Matches on ip+port rather than ip alone: the same handset reachable on two
   * ports is two entries, and forgetting one should not silently drop the
   * other. Disconnects first if the device being forgotten is the live one —
   * otherwise ADB keeps the socket open to a device the UI no longer lists.
   */
  ipcMain.removeHandler('adb-forget-device')
  ipcMain.handle('adb-forget-device', async (_, { ip, port }) => {
    const cleanIp = String(ip ?? '').trim()
    const cleanPort = String(port ?? '').trim()
    if (!cleanIp) return { success: false, error: 'An IP address is required.' }

    if (activeDevice && activeDevice.ip === cleanIp && activeDevice.port === cleanPort) {
      try {
        await execAsync(`adb disconnect ${cleanIp}:${cleanPort}`)
      } catch (_e) {
        // Already gone, or ADB is not installed. Forgetting still proceeds —
        // the entry is ours to delete regardless of what the daemon thinks.
      }
      activeDevice = null
    }
    ...
    try {
      ...
      const remaining = history.filter((d) => !(d.ip === cleanIp && String(d.port) === cleanPort))
      await fs.mkdir(dirPath, { recursive: true })
      await fs.writeFile(historyPath, JSON.stringify(remaining, null, 2))
      return { success: true, devices: remaining }
    } catch (e) {
