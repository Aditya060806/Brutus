...
import { configureTransformers } from '../services/voice/model-store'

let pipeline: any = null
let lancedb: any = null

/**
 * Load Transformers.js with a writable cache directory.
 *
 * Importing it is not enough. Its default cache is `__dirname/.cache`, which in
 * a packaged build is inside the read-only app.asar — so this used to work in
 * development and fail in the shipped installer. `configureTransformers` points
 * it at `userData/models` instead. See `services/voice/model-store.ts`.
 */
  ...
  ipcMain.handle('index-folder', async (event, folderPath: string) => {
    try {
      ...
      try {
        ...
        return `❌ Failed to load @xenova/transformers: ${String(e)}`
      }
      try {
        if (!lancedb) lancedb = await import('vectordb')
      } catch (e) {
        return `❌ Failed to load vectordb: ${String(e)}`
      }
      ...
      for (const file of filesToIndex) {
        try {
          ...
          console.warn(`Skipping file ${path.basename(file)} during indexing:`, e)
        }
      }
