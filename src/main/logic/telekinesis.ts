...
try {
  ...
  console.warn('node-window-manager not available. Telekinesis features will be disabled.', e)
}

export default function registerTelekinesis({ ipcMain }: { ipcMain: IpcMain }) {
  ipcMain.handle('teleport-windows', async (_event, commands) => {
    try {
      if (!windowManager) throw new Error('Telekinesis disabled: node-window-manager native module is missing.')
