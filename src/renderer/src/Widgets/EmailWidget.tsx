...
export default function EmailWidget() {
  ...
  return (
    <div className="fixed inset-0 z-9050 flex items-center justify-center bg-black/90 backdrop-blur-md p-10 animate-in fade-in zoom-in duration-300">
      <div className="relative w-full max-w-5xl h-[85vh] border-2 border-red-500/50 rounded-3xl overflow-hidden shadow-[0_0_100px_rgba(var(--brutus-accent-c),0.15)] bg-zinc-950 flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-white/5 bg-black/50 shrink-0 z-10">
          <div className="flex items-center gap-4">
            {selectedEmail ? (
              <button
                onClick={() => setSelectedEmail(null)}
                className="p-3 bg-white/5 hover:bg-red-500/20 text-zinc-400 hover:text-red-400 rounded-xl transition-all"
              >
                <RiArrowLeftLine size={24} />
              </button>
            ) : (
              <div className="p-3 bg-red-500/10 rounded-xl border border-red-500/20 shadow-[0_0_15px_rgba(var(--brutus-accent-c),0.2)]">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500 font-mono mt-1 uppercase">
                {selectedEmail
                  ...
                  : `INTERCEPTED BY BRUTUS // ${emails.length} MESSAGES`}
              </p>
            </div>
          </div>
          <button
                  ...
                  <div className="flex flex-col gap-4">
                    {emails.map((email) => (
                      <div
                        ...
                        className="group p-5 bg-white-[0.02] hover:bg-white-[0.05] border border-white/5 hover:border-red-500/30 rounded-2xl transition-all cursor-pointer relative overflow-hidden"
                      >
                        <div className="absolute inset-0 bg-linear-to-r from-red-500/0 via-red-500/0 to-red-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

                        <div className="relative z-10 flex flex-col gap-3">
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-2 text-red-400 text-xs font-mono bg-red-400/10 px-3 py-1.5 rounded-md border border-red-400/20 max-w-[60%]">
                  ...
                  {selectedEmail.attachments.length > 0 && (
                    <div className="mt-6 pt-4 border-t border-white/5 flex flex-wrap gap-3">
                      {selectedEmail.attachments.map((file, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2.5 rounded-lg hover:border-red-500/50 transition-colors cursor-default"
                        >
