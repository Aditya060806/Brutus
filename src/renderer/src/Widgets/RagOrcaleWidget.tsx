    ...
    <div className="absolute inset-0 z-999 flex items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-300 p-8">
      <AnimatePresence>
        <motion.div
          ...
          className="w-full max-w-4xl bg-canvas border border-red-500/30 rounded-2xl shadow-[0_0_100px_rgba(var(--brutus-accent-c),0.15)] overflow-hidden flex flex-col relative"
        >
          <motion.div
          ...
          <div className="h-16 bg-red-500/10 border-b border-red-500/20 flex items-center justify-between px-8">
            <div className="flex items-center gap-4">
              {status === 'scanning' && (
                <Loader2 className="w-6 h-6 text-red-400 animate-spin" />
              )}
              {status === 'ingesting' && (
                <Database className="w-6 h-6 text-red-400 animate-pulse" />
              )}
              {status === 'thinking' && (
                <BrainCircuit className="w-6 h-6 text-red-400 animate-pulse" />
              )}
              {status === 'done' && <CheckCircle2 className="w-6 h-6 text-red-400" />}
              {status === 'cancelled' && <Octagon className="w-6 h-6 text-red-500" />}
              <span
                className={`text-base font-black tracking-[0.2em] uppercase ${status === 'cancelled' ? 'text-red-500' : 'text-red-400'}`}
              >
                {status === 'scanning'
                  ? 'Scanning Folders'
                  : status === 'ingesting'
          ...
          <motion.div layout className="p-10 flex flex-col gap-8">
            {status === 'scanning' || status === 'ingesting' ? (
              <motion.div layout className="flex flex-col gap-8">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <TerminalSquare className="w-5 h-5 text-red-500/50" />
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Activity
                        className={`w-5 h-5 ${status === 'ingesting' && !isPaused ? 'text-red-400 animate-pulse' : 'text-red-500/50'}`}
                      />
                  ...
                  <div className="w-full h-3 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800 relative">
                    <motion.div
                      className="h-full bg-red-500 shadow-[0_0_15px_rgba(var(--brutus-accent-c),0.5)] rounded-full absolute top-0 left-0"
                      initial={{ width: 0 }}
                      animate={{ width: `${percentRaw}%` }}
                      transition={{ ease: 'easeInOut', duration: 0.3 }}
                    />
                    <motion.div
                      className="h-full bg-red-500/20 absolute top-0 left-0"
                      animate={{
                        width:
                ...
                <div
                  ref={terminalRef}
                  className="bg-canvas border border-zinc-800 rounded-xl p-6 h-45 overflow-y-auto relative font-mono text-sm scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent"
                >
                  {logStream.map((log, i) => (
                    <motion.div
                      key={i + log}
                      ...
                      className="text-red-400/80 mb-2 flex items-start gap-3"
                    >
              ...
              <motion.div layout className="flex flex-col items-center justify-center py-20">
                ...
                <div
                  className={`mt-8 text-sm font-mono flex flex-col items-center gap-2 ${status === 'cancelled' ? 'text-red-500/50' : 'text-red-500/50'}`}
                >
...
const StatBox = ({
  ...
  <div
    className={`flex flex-col justify-center p-5 rounded-xl border ${animate ? 'border-red-500/20 bg-red-500/5' : 'bg-black/40 border-white/5'}`}
  >
