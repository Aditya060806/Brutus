        ...
        <div className="flex items-center gap-1 px-6 pt-3">
          ...
          <div className="ml-auto flex items-center gap-2 pb-2">
            ...
            <input
              ...
              disabled={busy}
              className="w-32 bg-canvas border border-white/10 rounded-md px-2 py-1 text-xs text-cyan-200 outline-none focus:border-cyan-500/40"
            />
          </div>
        </div>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <div className="mt-2 flex gap-2">
          <input
            ...
            className="flex-1 bg-canvas border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 outline-none focus:border-cyan-500/40 disabled:opacity-50"
          />
          <button
            onClick={pickFolder}
            disabled={busy}
      ...
      {(busy || log.length > 0) && (
        <div
          ref={logRef}
          className="max-h-48 overflow-y-auto scrollbar-small bg-canvas border border-white/10 rounded-lg p-3 space-y-1.5"
        >
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="mt-2 flex gap-2">
          <input
            ...
            className="flex-1 bg-canvas border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 outline-none focus:border-violet-500/40 disabled:opacity-50"
          />
          <button
            onClick={browseVault}
            disabled={importing}
      ...
      {(importing || log.length > 0) && (
        <div
          ref={logRef}
          className="max-h-40 overflow-y-auto scrollbar-small bg-canvas border border-white/10 rounded-lg p-3 space-y-1.5"
        >
          {log.map((line: string, i: number) => (
            <div key={i} className="flex items-center gap-2 text-xs font-mono text-zinc-400">
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 bg-canvas border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 outline-none focus:border-cyan-500/40"
        />
        <button
          onClick={ask}
...
function GraphCanvas({ viz }: { viz?: { nodes: VizNode[]; edges: VizEdge[] } }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-canvas overflow-hidden">
