  ...
  const handleDone = contextSafe((e: any) => {
    ...
    if (progressRef.current) {
      gsap.to(progressRef.current, {
        width: '100%',
        // Both arms of this were '#ef4444', so the bar looked identical whether
        // the research succeeded or failed. Sage / coral, matching the status
        // colours the rest of the app uses.
        backgroundColor: success ? '#34c759' : '#ef4444',
        duration: 0.6,
        ease: 'power3.out'
      })
    }
      ...
      {isOpen && (
        ...
        >
          <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4 relative z-10">
            ...
            {isSuccess === true && (
              <CheckCircle2 className="w-6 h-6 text-red-500 shadow-[0_0_20px_rgba(var(--brutus-accent-c),0.5)] rounded-full" />
            )}
          </div>
          ...
          {summary && (
            <div
              ref={summaryRef}
              className="mt-4 p-4 bg-red-950/20 border border-red-500/20 rounded-lg relative z-10 max-h-37.5 overflow-hidden"
            >
              <p className="text-[10px] text-red-400/80 uppercase tracking-widest mb-2 font-bold">
                Data Extracted
              </p>
