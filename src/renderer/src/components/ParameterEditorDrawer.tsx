    ...
    <div className="flex flex-col gap-2 relative w-full">
      <div className="relative w-full">
        ...
        <input
          ...
          className="w-full bg-canvas border border-line rounded-md text-xs py-2 pl-9 pr-3 text-white outline-none focus:border-red-500 transition-colors placeholder-zinc-700"
        />
      </div>

      <div className="flex flex-col gap-1.5 h-120 overflow-y-auto scrollbar-small bg-canvas border border-line rounded-md p-1.5 w-full shadow-inner">
        ...
        {visibleApps.map((app, index) => {
          ...
          const AppRow = (
            <div
              ...
              className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-all duration-300 transform active:scale-95 group ${isSelected ? 'bg-red-500/15 border border-red-500/60 shadow-[0_0_15px_rgba(var(--brutus-accent-c),0.15)] order-first' : 'hover:bg-elevated border border-transparent hover:border-white/5'}`}
            >
              ...
              {isSelected && (
                <RiCheckLine
                  className="text-red-500 shrink-0 mx-2 animate-in zoom-in-50 duration-200"
                  size={18}
                />
              )}
            </div>
...
export default function ParameterEditorDrawer({ nodeData, updateNodeInputs, closeEditor }: any) {
  ...
  return (
    <div className="absolute top-0 right-0 w-80 h-full bg-surface border-l border-line shadow-2xl flex flex-col z-50 animate-in slide-in-from-right-8 duration-200">
      <div className="p-4 border-b border-line flex justify-between items-center bg-elevated">
      ...
      <div className="p-5 grow overflow-y-auto flex flex-col gap-6 custom-scrollbar scrollbar-small">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            className="bg-canvas border border-line rounded-md text-xs p-2.5 text-white outline-none focus:border-red-500 transition-colors placeholder-zinc-700 shadow-inner"
            value={localComment}
            ...
            Object.entries(tool.parameters.properties).map(([key, prop]: any) => (
              <div key={key} className="flex flex-col gap-2 w-full">
                ...
                {key === 'app_name' && (tool.name === 'open_app' || tool.name === 'close_app') ? (
                  ...
                  <select
                    className="bg-canvas border border-line rounded-md text-xs p-2.5 text-white outline-none focus:border-red-500 transition-colors cursor-pointer w-full"
                    value={localInputs[key] || ''}
                  ...
                  <input
                    ...
                    className="bg-canvas border border-line rounded-md text-xs p-2.5 text-white outline-none focus:border-red-500 transition-colors placeholder-zinc-700 font-mono shadow-inner w-full"
                    value={localInputs[key] || ''}
                    onChange={(e) => handleInputChange(key, e.target.value)}
                  />
                )}
      ...
      <div className="p-4 border-t border-line bg-elevated">
        <button
          onClick={handleSave}
          className="w-full bg-red-600 hover:bg-red-500 text-black py-2.5 rounded-lg text-[11px] font-black tracking-widest transition-all shadow-[0_0_15px_rgba(var(--brutus-accent-c),0.2)] flex items-center justify-center gap-2 cursor-pointer"
        >
