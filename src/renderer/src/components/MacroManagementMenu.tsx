  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className={`flex items-center gap-3 px-4 py-2 bg-elevated border rounded-lg text-sm text-zinc-300 font-medium transition-all cursor-pointer ${isMainOpen ? 'border-red-500 shadow-[0_0_15px_rgba(var(--brutus-accent-c),0.3)]' : 'border-line hover:border-zinc-700'}`}
      >
      ...
      {isMainOpen && (
        <div className="absolute top-12 left-0 w-80 bg-surface border border-line rounded-xl shadow-2xl z-50 p-2 flex flex-col gap-1 max-h-96 overflow-y-auto scrollbar-small animate-in fade-in duration-200">
          <h4 className="text-[10px] font-black tracking-widest text-zinc-600 p-2 uppercase border-b border-line mb-2">
            INGESTED MACROS
          </h4>
          ...
          {workflows.map((macro: any) => (
            <div key={macro.name} className="relative group">
              ...
              >
                <span className="text-xs font-bold text-zinc-100 uppercase tracking-wide group-hover:text-red-400">
                  {macro.name}
                </span>
              ...
              {activeWorkflowActions === macro.name && (
                <div className="absolute top-8 right-2 w-32 bg-black border border-line rounded-lg shadow-xl z-20 p-1 flex flex-col animate-in scale-95 fade-in duration-100">
                  {[
