  ...
  return (
    <div className="w-full h-full flex items-center justify-center px-2 bg-black/95 backdrop-blur-2xl rounded-2xl border border-white/[0.06] shadow-[0_4px_30px_rgba(0,0,0,0.8),inset_0_1px_0_rgba(255,255,255,0.03)] drag-region overflow-hidden">
      <div className="flex items-center gap-1 no-drag">
        <div
          className={`w-7 h-7 rounded-xl flex items-center justify-center transition-all duration-500 ${isSystemActive ? (isTalking ? 'bg-red-500/20 shadow-[0_0_12px_rgba(239,68,68,0.4)]' : 'bg-red-500/10') : 'bg-white/[0.03]'}`}
        >
          <div
            className={`w-2 h-2 rounded-full transition-all duration-500 ${isSystemActive ? (isTalking ? 'bg-red-400 shadow-[0_0_6px_#ef4444]' : 'bg-red-600') : 'bg-zinc-700'}`}
          />
        </div>
        ...
        <button
          ...
          className={`p-2 rounded-xl border transition-all duration-500 mx-0.5 ${isSystemActive ? 'bg-red-500/15 border-red-500/40 text-red-400 shadow-[0_0_20px_rgba(239,68,68,0.15)]' : 'bg-white/[0.03] border-white/[0.06] text-zinc-500 hover:text-white hover:border-white/10'}`}
        >
