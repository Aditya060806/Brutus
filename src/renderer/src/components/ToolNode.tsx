...
export const getIcon = (name: string, size = 16) => {
  ...
  if (name.includes('terminal') || name.includes('code') || name.includes('app'))
    return <RiTerminalBoxLine size={size} className="text-red-400" />
...
export default function ToolNode({ data, id }: any) {
  ...
  return (
    <div
      className="bg-elevated border border-line rounded-lg shadow-xl min-w-50 max-w-62.5 font-sans text-zinc-100 group transition-all hover:border-red-500/50 relative"
    ...
    >
      ...
      <div className="flex items-center justify-between p-2.5">
        <div className="flex items-center gap-3">
          <div className="p-1.5 bg-canvas rounded shadow-inner border border-white/5">
      ...
      {isTrigger && (
        <Handle
          ...
          id="source-bottom"
          className="w-4 h-2.5 bg-red-500 rounded-sm border-none -mb-1"
        />
      )}
      {isWait && (
        <>
          <Handle
            ...
            id="source-right"
            className="w-2.5 h-4 bg-red-500 rounded-sm border-none -mr-1"
          />
          <Handle
            type="source"
            position={Position.Bottom}
