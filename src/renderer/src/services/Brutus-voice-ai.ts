...
export class GeminiLiveService {
  ...
  public engine: 'cloud' | 'server' = 'cloud'
  ...
  private updateState() {
    if (!this.isConnected) {
      this.state = 'idle'
      this.emotion = 'sleepy'
    } else if (this.activeAudioNodes.length > 0) {
      this.state = 'speaking'
      this.emotion = 'happy'
  ...
  private async finalizeEdgeTurn(): Promise<void> {
    ...
    try {
      ...
      const asr = await window.electron.ipcRenderer.invoke('brain-asr', { wavBase64 })
  ...
  private async edgeRespond(userText: string, persist: boolean): Promise<void> {
    ...
    try {
      ...
      const tts = await window.electron.ipcRenderer.invoke('brain-tts', { text: reply })
      this._isProcessingTools = false
  ...
  private handleSocketClose(): void {
    ...
    if (this.reconnectAttempts < this.MAX_RECONNECT_ATTEMPTS) {
      ...
      this.reconnectTimer = setTimeout(() => {
        this.connect().catch((e) => console.error('[Brutus] Reconnect attempt failed:', e))
      }, delay)
    } else {
