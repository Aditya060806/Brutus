 ...
 */
// Panels sit on a pure-black canvas, so they are defined by a hairline and a
// barely-lighter fill rather than by a shadow — there is nothing darker than
// the canvas for a shadow to fall on.
const SURFACE = 'bg-surface/80 backdrop-blur-xl border border-line rounded-2xl'
...
export default function DashboardView({
  ...
  const handleSendText = async () => {
    ...
    // `/agent` runs the multi-agent orchestrator and works even when the voice
    // session is offline, since it never touches the live socket.
    if (isAgentCommand(msg)) {
      const request = stripAgentCommand(msg)
      setTextInput('')
      if (!request) {
        window.dispatchEvent(new CustomEvent('open-orchestrator'))
        return
      }
      await saveMessage('user', msg)
      const started = await orchestrator.run(request)
      if (!started.ok) {
        await saveMessage('brutus', `Could not start agents: ${started.error}`)
        return
      }
      window.dispatchEvent(new CustomEvent('open-orchestrator'))
      const { answer } = await orchestrator.waitForAnswer()
      await saveMessage('brutus', answer)
      return
    }
        ...
        <div className="absolute bottom-8 z-50">
          ...
          >
            ...
            >
              ...
              {/* Red ONLY while the session is live. An idle call button that
                  glows red reads as "recording" when nothing is recording —
                  which is the one thing this control must never imply. Idle is
                  a neutral outline that warms on hover. */}
              <div
                className={`relative p-3.5 rounded-full border transition-all duration-500 ${
                  isSystemActive
                    ? 'bg-primary-500 border-primary-400 text-black shadow-[0_0_28px_rgba(var(--brutus-accent-c),0.55)]'
                    : 'bg-white/[0.04] border-line-strong text-content-secondary group-hover:border-primary-500/50 group-hover:text-primary-400'
                }`}
              >
                <RiPhoneFill size={22} />
              </div>
            ...
            {chatHistory.length === 0 ? (
              ...
              chatHistory.map((msg, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Your messages are lighter, Brutus's are darker — the
                        contrast between them carries the distinction. Tinting
                        one side red said "warning" about ordinary chat. */}
                    <div
                      className={`max-w-[92%] px-3 py-2 text-[11px] leading-relaxed rounded-xl ${
                        mine
                          ? 'bg-white/[0.09] text-content rounded-br-md'
                          : 'bg-white/[0.035] text-content-secondary rounded-bl-md'
                      }`}
                    >
          ...
          <div className="px-3 pb-3 pt-2.5 border-t border-white/[0.06] shrink-0">
            <div className="flex items-end gap-2">
              <textarea
                ...
                // Never disabled: /agent runs the orchestrator, which does not
                // need the live voice session. The non-agent path still checks
                // isSystemActive before sending.
                placeholder={isSystemActive ? 'Message Brutus' : 'Offline — /agent still works'}
                rows={1}
                className="flex-1 bg-canvas border border-line rounded-lg text-[11px] text-content placeholder-content-faint px-3 py-2 outline-none resize-none transition-colors focus:border-line-strong disabled:opacity-40 disabled:cursor-not-allowed"
              />
              {/* Neutral. Sending a message is not an accent-worthy act; the
                  button earns emphasis by becoming legible when it is usable,
                  not by being red the whole time. */}
              <motion.button
                whileTap={reduce ? undefined : { scale: 0.92 }}
                onClick={handleSendText}
                disabled={!textInput.trim() || (!isSystemActive && !isAgentCommand(textInput))}
                title="Send"
                className="p-2 rounded-lg bg-white/[0.06] text-content-secondary transition-colors hover:bg-white/[0.12] hover:text-content disabled:opacity-25 disabled:cursor-not-allowed cursor-pointer"
              >
