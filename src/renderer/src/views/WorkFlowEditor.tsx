...
function Editor() {
  ...
  const [workflowName, setWorkflowName] = useState('New BRUTUS Macro')
  ...
  const resetCanvas = () => {
    setWorkflowName('New BRUTUS Macro')
        ...
        addEdge(
          {
            ...
            type: 'default',
            animated: true,
            style: { stroke: '#ef4444', strokeWidth: 2, filter: 'drop-shadow(0 0 4px #ef4444)' }
          },
          eds
        )
      ),
  ...
  return (
    <div className="flex h-full w-full bg-canvas relative overflow-hidden animate-in fade-in duration-300">
      <div
        className={`fixed top-14 left-0 h-[calc(100vh-56px)] bg-surface border-r border-line p-4 flex flex-col gap-1 transition-all duration-300 ease-in-out z-40 scrollbar-small overflow-auto mt-5 ${isSidebarOpen ? 'w-72 opacity-100' : 'w-0 opacity-0'}`}
      >
        {isSidebarOpen && (
          <>
            <h2 className="text-[10px] font-black tracking-[0.2em] text-red-500 mb-6 flex items-center gap-2 border-b border-line pb-2 uppercase">
              MODULE LIBRARY
            </h2>
              ...
              <div key={category} className="mb-6">
                ...
                <div className="flex flex-col gap-2">
                  {tools.map((tool: any) => (
                    <div
                      key={tool.name}
                      className="flex items-center gap-3 p-2 bg-elevated border border-line rounded-lg cursor-grab hover:border-red-500/50 hover:bg-line/50 transition-all group"
                      draggable
                      onDragStart={(e) =>
      ...
      <button
        ...
        className="absolute top-1/2 left-0 transform -translate-y-1/2 bg-surface border border-line border-l-0 p-2 rounded-r-lg text-zinc-600 hover:text-red-500 z-50 transition-colors"
      >
      ...
      >
        <div className="absolute top-4 left-4 z-10 flex items-center gap-3 shadow-2xl">
          <button
            onClick={resetCanvas}
            className="p-3 rounded-lg bg-elevated border border-line text-zinc-600 hover:text-red-500 hover:border-red-500/50 transition-colors cursor-pointer"
          ...
          <input
            ...
            className="bg-elevated border border-line px-4 py-2 rounded-lg text-sm text-white outline-none focus:border-red-500 font-bold tracking-wide w-64 shadow-inner"
          />

          <button
            onClick={runMacroManually}
            className="bg-elevated hover:bg-line text-red-400 px-5 py-2 rounded-lg text-[11px] font-black tracking-widest transition-all border border-line hover:border-red-500/50 flex items-center gap-2 cursor-pointer shadow-lg"
          >
          ...
          <button
            ...
            className="bg-red-600 hover:bg-red-500 text-black px-6 py-2 rounded-lg text-[11px] font-black tracking-widest transition-all shadow-[0_0_20px_rgba(var(--brutus-accent-c),0.3)] flex items-center gap-2 cursor-pointer"
          >
