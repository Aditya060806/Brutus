...
const MarkdownComponents = {
  code({ node, inline, className, children, ...props }: any) {
    return !inline ? (
      ...
      <code
        className="bg-white/10 px-1 py-0.5 rounded text-red-400 font-mono text-xs"
        {...props}
      >
        {children}
      </code>
      ...
      <div className="col-span-4 flex flex-col gap-4 h-full overflow-hidden">
        <div className="flex items-center justify-between pb-2 border-b border-white/10">
          <div className="flex items-center gap-2 text-zinc-100">
            <RiStickyNoteLine className="text-red-400" />
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-2 scrollbar-small">
          {notes.length === 0 ? (
            <div className="text-center text-zinc-400 text-xs mt-10">
              ...
              <p className="mt-2 opacity-50">Click + or ask BRUTUS.</p>
            </div>
          ) : (
            notes.map((note) => (
              <div
                ...
                className={`group relative overflow-hidden p-3 rounded-xl border transition-all duration-300 cursor-pointer flex items-center justify-between hover:-translate-y-0.5 ${
                  selectedNote?.filename === note.filename && !isEditorOpen
                    ? 'bg-red-500/10 border-red-500/30 shadow-[0_0_15px_rgba(var(--brutus-accent-c),0.1)]'
                    : 'bg-zinc-900/40 border-white/5 hover:bg-white/5 hover:border-white/10 hover:shadow-[0_6px_18px_rgba(var(--brutus-accent-c),0.08)]'
                }`}
              >
                <span
      ...
      >
        {isEditorOpen ? (
          <div className="flex-1 flex flex-col p-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                className="flex items-center gap-2 px-6 py-2 bg-red-500 text-black font-bold text-xs rounded-lg hover:bg-red-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-zinc-200 gap-5 animate-in fade-in duration-500">
            ...
            <button
              onClick={startCreating}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/10 border border-red-500/25 text-red-400 text-[11px] font-bold tracking-widest hover:bg-red-500/20 hover:border-red-500/40 hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(var(--brutus-accent-c),0.15)] transition-all duration-300"
            >
