...
const PhoneView = ({ glassPanel }: { glassPanel?: string }) => {
  ...
  const handleManualConnect = () => {
    localStorage.setItem('brutus_adb_ip', ip)
    localStorage.setItem('brutus_adb_port', port)
  ...
  if (status !== 'connected' && uiMode === 'history') {
    return (
      <div className="flex-1 flex flex-col items-center justify-start pt-16 p-10 animate-in fade-in duration-300 bg-[#050505] min-h-screen text-red-50 relative overflow-y-auto scrollbar-small pb-24">
        <div className="w-full max-w-6xl flex flex-col items-center">
          <div className="flex flex-col items-center text-center mb-16">
            <div className="p-4 bg-red-500/10 rounded-2xl border border-red-500/30 mb-6 inline-block">
              <RiHistoryLine className="text-red-400" size={32} />
            </div>
            <h1 className="text-4xl font-black text-white tracking-[0.2em] uppercase">
              NEURAL ARCHIVE
            </h1>
            <p className="text-xs text-red-500 font-mono tracking-widest mt-2">
          ...
          <div className="flex flex-wrap justify-center gap-10">
            {deviceHistory.map((dev, i) => (
              ...
              >
                ...
                <div className="flex-1 bg-linear-to-b from-zinc-900 to-black rounded-[2.2rem] overflow-hidden flex flex-col items-center justify-center p-6 relative">
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-mono text-zinc-500 group-hover:text-red-300 z-10">
                    <RiWifiLine /> {dev.ip}:{dev.port}
                  </div>
                  <div className="mt-8 px-6 py-2 border border-zinc-700 group-hover:border-red-500 bg-transparent group-hover:bg-red-500 text-zinc-500 group-hover:text-black font-bold text-[10px] tracking-widest rounded-full transition-all z-10">
            ...
            <button
              ...
              className="w-55 h-110 bg-transparent border-4 border-dashed border-zinc-800 hover:border-red-500/50 rounded-[3rem] flex flex-col items-center justify-center group transition-all duration-500 hover:bg-red-500/5"
            >
              <div className="w-16 h-16 rounded-full bg-zinc-900 group-hover:bg-red-500 flex items-center justify-center text-zinc-500 group-hover:text-black transition-all mb-4">
                <RiAddLine size={32} />
              </div>
              <span className="text-xs font-bold text-zinc-500 group-hover:text-red-400 tracking-widest uppercase">
                NEW DEVICE
              </span>
            </button>
          </div>
  ...
  if (status !== 'connected' && uiMode === 'manual') {
    return (
      <div className="flex-1 flex flex-col lg:flex-row items-start justify-center gap-8 p-6 md:p-12 animate-in fade-in duration-300 bg-[#050505] min-h-dvh overflow-y-auto text-red-50 pb-24">
        <div className="w-full lg:w-1/3 max-w-md flex flex-col gap-6 shrink-0">
          <div className="p-6 bg-black border border-red-900/40 rounded-2xl shadow-lg flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-red-950/40 rounded-xl border border-red-400/30">
                <FaAndroid className="text-red-400 text-2xl" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-white tracking-wide">Device Uplink</h2>
                <p className="text-[10px] text-red-400/70 font-mono">TCP/IP CONFIGURATION</p>
              </div>
            </div>
            ...
            {deviceHistory.length > 0 && (
              <button
                ...
                className="text-[10px] font-bold tracking-widest text-red-500 hover:text-red-300 hover:bg-red-500/10 uppercase px-3 py-1.5 border border-red-500/30 rounded-lg transition-all shrink-0 ml-2"
              >
                ARCHIVE
              </button>
            )}
          ...
          <div
            className={`${glassPanel || 'bg-zinc-950'} p-8 border border-red-900/40 rounded-2xl shadow-lg flex flex-col gap-6`}
          >
            {errorMsg && (
            ...
            <div>
              <label className="text-xs font-bold text-red-400/80 tracking-wide mb-3 block">
                Target IP Address
              </label>
              <div className="flex items-center bg-black border border-red-900/50 rounded-xl px-5 py-4 focus-within:border-red-400 transition-all">
                <RiWifiLine className="text-red-400 mr-3" size={20} />
                <input
                  type="text"
                  value={ip}
                  onChange={(e) => setIp(e.target.value)}
                  placeholder="192.168.1.xxx"
                  className="bg-transparent border-none outline-none text-base text-red-400 w-full font-mono placeholder:text-red-900/50"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center bg-black border border-red-900/50 rounded-xl px-5 py-4 focus-within:border-red-400 transition-all">
                <RiLinkM className="text-red-400 mr-3" size={20} />
                <input
                  type="text"
                  value={port}
            ...
            <button
              ...
              className="w-full mt-4 py-5 bg-red-950 border border-red-400/50 hover:bg-red-400 text-red-400 hover:text-black font-bold rounded-xl tracking-widest transition-all duration-300 uppercase text-sm"
            >
        ...
        <div className="w-full lg:w-1/2 max-w-2xl flex flex-col">
          <div className="bg-black border border-red-900/40 rounded-2xl shadow-lg p-8 md:p-10 flex flex-col relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none">
              <RiTerminalLine size={240} />
            </div>

            <div className="flex items-center gap-4 mb-8 relative z-10">
              <RiTerminalBoxLine className="text-red-500" size={28} />
              <h3 className="text-base font-bold tracking-[0.2em] text-red-400 uppercase">
            ...
            <div className="space-y-8 relative z-10">
              <div className="flex gap-5">
                <div className="flex flex-col items-center">
                  <div className="w-8 h-8 rounded-full bg-red-950 border border-red-500/50 flex items-center justify-center text-xs font-bold text-red-400 shrink-0">
                    1
                  </div>
                  <div className="w-px h-full bg-red-900/30 my-2"></div>
                </div>
                <div className="pb-3">
                  ...
                  <p className="text-xs font-mono text-zinc-500 leading-relaxed">
                    Go to{' '}
                    <span className="text-red-400/70">Settings &gt; Developer Options</span> on
              ...
              <div className="flex gap-5">
                ...
                <div className="pb-3 w-full">
                  ...
                  <div className="relative group w-full">
                    <code className="block w-full bg-zinc-950 border border-red-900/30 text-red-400 text-sm p-4 pr-14 rounded-xl tracking-widest font-mono">
                      adb tcpip 5555
                    </code>
                    <button
                      onClick={handleCopyCommand}
                      className="absolute right-3 top-1/2 -translate-y-1/2 p-2 text-red-600 hover:text-red-400 hover:bg-red-900/30 rounded-lg transition-all"
                      title="Copy command"
                    >
                      {copied ? (
                        <RiCheckLine size={20} className="text-red-400" />
                      ) : (
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-[#111] border border-white/5 rounded-2xl p-5 hover:border-purple-500/30 transition-all">
            ...
            <div className="w-full bg-zinc-800 rounded-full h-1.5 overflow-hidden">
              <div
                className="bg-red-500 h-1.5 shadow-[0_0_10px_rgba(239,68,68,0.8)]"
